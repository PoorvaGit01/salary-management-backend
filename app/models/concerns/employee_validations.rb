module EmployeeValidations
  ...
  included do
    validates :first_name, :last_name, presence: true, length: { maximum: Employee::MAX_NAME_LENGTH }
    validates :job_title, presence: true, length: { maximum: Employee::MAX_TITLE_LENGTH }
    validates :department, length: { maximum: Employee::MAX_TITLE_LENGTH }, allow_blank: true

    validates :country, presence: true, length: { is: 2 }, format: {
      with: /\A[A-Z]{2}\z/,
      message: "must be a 2-letter ISO country code"
    }
    validates :currency, presence: true, length: { is: 3 }, format: {
      with: /\A[A-Z]{3}\z/,
    ...
    validates :salary, numericality: {
      greater_than: 0,
      less_than_or_equal_to: Employee::MAX_SALARY
    }

    validates :email,
              uniqueness: { allow_blank: true },
              length: { maximum: Employee::MAX_EMAIL_LENGTH },
              format: { with: URI::MailTo::EMAIL_REGEXP, allow_blank: true }

    validates :employee_number,
              uniqueness: { allow_blank: true },
              length: { maximum: Employee::MAX_EMPLOYEE_NUMBER_LENGTH },
              format: {
